...
远程使用 Claude Code 时，自动获得推送通知。支持 Bark（桌面）和企业微信（手机）两级推送。
...
Claude Code 事件触发
        │
        ▼
  notify.sh 清除旧 pending → 创建新 pending → 后台倒计时
        │
        ├── 15s 后 ──→ pending 存在？──→ 🔔 Bark（桌面通知）
        │                  └── 不存在 ──→ 跳过
        │
        └── 5min 后 ──→ pending 存在？──→ 💬 企业微信（手机兜底）
                           └── 不存在 ──→ 跳过

  取消推送的 3 种方式：
  ├── 新事件进来（如权限确认后 Claude 继续工作触发 Stop）→ 清旧 pending
